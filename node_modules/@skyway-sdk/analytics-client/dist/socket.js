...
class Socket {
    ...
    _connect() {
        ...
        ws.onclose = (event) => {
            ...
            const isInternalReconnectClose = event.code === 4000 && this._reconnectClosingWebSocket === ws;
    ...
    reconnect() {
        if (this._ws !== undefined &&
            (this._ws.readyState === isomorphic_ws_1.default.CONNECTING ||
                this._ws.readyState === isomorphic_ws_1.default.OPEN)) {
    ...
    _pushResendClientEvent(data) {
        ...
        if (this._resendClientEvents.length >= Socket.MAX_RESEND_CLIENT_EVENTS) {
            ...
            if (!this._hasWarnedResendQueueOverflow) {
                this._logger.warn(`Dropped oldest resend event because queue exceeded limit (${Socket.MAX_RESEND_CLIENT_EVENTS}). Further overflow warnings are suppressed until reconnect succeeds.`);
...
Socket.MAX_RESEND_CLIENT_EVENTS = 200;
