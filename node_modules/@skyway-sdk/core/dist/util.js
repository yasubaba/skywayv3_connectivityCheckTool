        ...
        if (channel.localPerson) {
            ...
            const publishing = yield Promise.all(member.publications.map((p) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (p.stream) {
                    for (const { memberId, stats } of yield p.stream._getStatsAll()) {
                        ...
                        const rawStats = yield ((_d = p.getRTCPeerConnection(memberId)) === null || _d === void 0 ? void 0 : _d.getStats());
                        if (rawStats) {
                            const stats = statsToArray(rawStats);
                            const localCandidates = stats.filter((s) => s.type === 'local-candidate');
                            const remoteCandidates = stats.filter((s) => s.type === 'remote-candidate');
            ...
            const subscribing = yield Promise.all(member.subscriptions.map((s) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (s.stream) {
                    ...
                    const rawStats = yield ((_e = s.getRTCPeerConnection()) === null || _e === void 0 ? void 0 : _e.getStats());
                    if (rawStats) {
                        const stats = statsToArray(rawStats);
                        const localCandidates = stats.filter((s) => s.type === 'local-candidate');
                        const remoteCandidates = stats.filter((s) => s.type === 'remote-candidate');
